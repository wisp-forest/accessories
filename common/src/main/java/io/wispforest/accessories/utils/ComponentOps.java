...
public class ComponentOps {
    ...
    public static ExtraInfoFooter attemptToAddExtraInfoFooter(SequencedCollection<String> baseKeys, TooltipFlag baseFlag, Function<TooltipFlag, Boolean> comparisonFunc) {
        ...
        for (var bitmask : bitmasks) {
            var newFlag = ((TooltipFlagExtended) baseFlag).withMask(bitmask);
            var hasChanged = comparisonFunc.apply(newFlag);

            if (hasChanged) {
                if (((TooltipFlagExtended) newFlag).hasShiftDown() && !((TooltipFlagExtended) baseFlag).hasShiftDown()) keys.add("key.keyboard.left.shift");
                if (((TooltipFlagExtended) newFlag).hasAltDown() && !((TooltipFlagExtended) baseFlag).hasAltDown()) keys.add("key.keyboard.left.alt");
                if (((TooltipFlagExtended) newFlag).hasControlDown() && !((TooltipFlagExtended) baseFlag).hasControlDown()) keys.add("key.keyboard.left.control");
            }
