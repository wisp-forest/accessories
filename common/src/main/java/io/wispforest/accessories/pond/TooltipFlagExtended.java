...
public interface TooltipFlagExtended {
    ...
    static TooltipFlag create(boolean isAdvanced) {
        return new NestedTooltipFlag(isAdvanced ? TooltipFlag.Default.ADVANCED : TooltipFlag.Default.NORMAL, AccessoriesClientInternals.getInstance().createBitFlag());
    }
}

record NestedTooltipFlag(TooltipFlag flag, int modifiers) implements TooltipFlag, TooltipFlagExtended {
    ...
    public TooltipFlag getInnerFlag() {
        var baseFlag = ((TooltipFlagExtended) flag).getInnerFlag();
